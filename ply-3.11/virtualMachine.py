...
class VirtualMachine():
    ...
    # def rebuildPar(self):
    #     with open ('pendientes.txt', 'r') as pendingList:
    #         self.pending
    #         for p in pendingList:
    #             self.pending.append(eval(p)) 
    # #             print('Pending List:', self.pending[-1])
            
    #         for p in self.pending:
    #             print('Pending List from self.pending p[1]:', p[1])
    #             self.pendingDirection.append(p[1])
    #             print('pendingDiection (p[1]) ---> ', p[1])
    ...
    def param(self, quad):
        if quad[2] == None:
            self.memoria.value_to_memory(quad[3], self.memoria.value_from_memory(quad[1])) 
            # print('self.pendingDirection[1] en param---', self.pendingDirection[1], quad[1])
            # print('self.memoria.value_to_memory(quad[1], self.pendingDirection[1]-->', self.memoria.value_to_memory(self.pendingDirection[1], quad[1]))
            # self.memoria.value_to_memory(quad[1], self.pendingDirection[1])
            self.ip +=1
