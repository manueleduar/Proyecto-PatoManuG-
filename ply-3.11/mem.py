...
class Memory:
    ...
    def set_cte(self, val):
        if isinstance(val, int):
            if(self.ctei < 46000):
                address = self.ctei
                print("constante entera se ha configurado con dir ", address)
                self.ctei += 1
                print("constante entera updated ", self.ctei)
        elif isinstance(val, float):
            if self.ctef < 47000:
                address = self.ctef
                print("constante flotante se ha configurado con dir", address)
                self.ctef += 1
            ...
            if len(val)<2:
                if self.ctec < 48000:
                    ...
                    print("constante char se ha configurado con dir ", address)
                    self.ctec += 1
                    print("constante char updated ", self.ctec)
            else:
                if self.cteString < 49000:
                    address = self.cteString
                    print("constante string se ha configurado con dir ", address)
                    self.cteString += 1
